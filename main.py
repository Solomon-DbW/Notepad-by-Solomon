...
def main():
    window = tk.Tk()
    window.title("SoloPad")
    window.rowconfigure(0, minsize=400)
    window.columnconfigure(1, minsize=500)
    # window.config(bg="#000") # Set the background color to black

    text_edit = tk.Text(window, font="Arial,sans-serif 18")  # fg="ffffff"
    ...
    window.bind("<Control-s>", lambda x: save_file(window, text_edit)) #Key shortcut
